...
**Cherry Terminal v1** is a pure native C++20 desktop application. It uses **Qt6** for UI and rendering, embedded **Python** for analytics, and delivers professional terminal-class performance in a single native binary.
...
```bash
./build/<preset>/Cherry-Terminal         # Linux / macOS (preset build)
.\build\<preset>\Cherry-Terminal.exe     # Windows (preset build)
```
